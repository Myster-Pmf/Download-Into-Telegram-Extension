...
const DEFAULT_SETTINGS = {
  backendUrl: 'https://lightx99-downloadintotelegram.hf.space',
...
// --- Browser Cookies Extraction Helpers ---
    ...
    try {
      chrome.cookies.getAll({ url: pageUrl }, (cookies) => {
        ...
        cookies.forEach(c => {
          ...
          lines.push(`${prefix}${domain}\t${flag}\t${path}\t${secure}\t${expiration}\t${c.name}\t${c.value}`);
        });
        resolve(lines.join('\n'));
      });
    } catch (e) {
      console.error('[Cookies] Extraction error:', e);
      resolve('');
    }
  });
}
    ...
    try {
      chrome.cookies.getAll({ domain }, (cookies) => {
        ...
        cookies.forEach(c => {
          ...
          const expiration = c.expirationDate ? Math.round(c.expirationDate) : 0;
          const prefix = c.httpOnly ? '#HttpOnly_' : '';
          lines.push(`${prefix}${c.domain}\t${flag}\t${path}\t${secure}\t${expiration}\t${c.name}\t${c.value}`);
        });
        resolve(lines.join('\n'));
      });
    } catch (e) {
      console.error('[Cookies] Domain extraction error:', e);
      resolve('');
    }
  });
}
...
function updateGlobalCookieStatusUI(liveText, savedText) {
  const dot = elements.globalCookieStatus.querySelector('.cookie-status-dot');
  const txt = elements.globalCookieStatus.querySelector('.cookie-status-text');
  
  const liveCount = countCookies(liveText);
  const savedCount = countCookies(savedText);

  elements.globalCookieStatus.className = 'cookie-status-indicator';

  if (!savedText) {
    if (liveCount > 0) {
      elements.globalCookieStatus.classList.add('has-cookies');
      txt.innerText = `Live Sync (${liveCount} active browser cookies)`;
    } else {
      elements.globalCookieStatus.classList.add('no-cookies');
      txt.innerText = 'Live Sync (No browser cookies detected)';
    }
    elements.globalCookieMismatchHint.style.display = 'none';
  } else {
    if (savedText.trim() === liveText.trim()) {
      elements.globalCookieStatus.classList.add('synced');
      txt.innerText = `Synced (${savedCount} cookies matched with browser)`;
      elements.globalCookieMismatchHint.style.display = 'none';
    } else {
      elements.globalCookieStatus.classList.add('override-cookies');
      txt.innerText = `Static Override (${savedCount} custom cookies)`;
      elements.globalCookieMismatchHint.style.display = 'block';
    }
  }
}

function toggleGlobalMismatchHint(currentText, liveText) {
...
async function checkModalDomainCookies(pattern) {
  if (!pattern) {
    ...
    elements.profLiveCookieStatus.querySelector('.cookie-status-text').innerText = 'No live cookies';
    return;
  }
  ...
  elements.profLiveCookieStatus.className = 'cookie-status-indicator';
  if (count > 0) {
    elements.profLiveCookieStatus.classList.add('has-cookies');
    txt.innerText = `${count} live cookies available in browser`;
  } else {
    elements.profLiveCookieStatus.classList.add('no-cookies');
    txt.innerText = 'No live cookies found in browser';
  }
}

async function getCookiesForDownload(video, matchedProfile) {
  if (matchedProfile) {
    if (matchedProfile.cookiesText) {
      return matchedProfile.cookiesText;
    } else {
      return await extractBrowserCookiesForDomain(matchedProfile.domainPattern);
    }
  } else {
    if (globalCookiesText) {
      return globalCookiesText;
    } else {
      return await extractBrowserCookies(video.pageUrl);
    }
  }
}
...
document.addEventListener('DOMContentLoaded', async () => {
  chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
    ...
    // Toggle global cookies editor visibility
    elements.globalEditCookiesBtn.addEventListener('click', () => {
      elements.globalCookieEditor.classList.toggle('active');
      const isActive = elements.globalCookieEditor.classList.contains('active');
      elements.globalEditCookiesBtn.innerText = isActive ? 'Collapse' : 'Edit';
    });

    // Refresh cookies from current browser tab
    elements.globalRefreshCookiesBtn.addEventListener('click', async () => {
      elements.globalRefreshCookiesBtn.disabled = true;
      elements.globalRefreshCookiesBtn.innerText = 'Syncing...';
      await refreshLiveCookiesState();
      elements.globalRefreshCookiesBtn.disabled = false;
      elements.globalRefreshCookiesBtn.innerText = '↻ Sync';
      showToast('Cookies synchronized with active tab.', 'success');
    });

    // Download/Export cookies.txt
    elements.globalDownloadCookiesBtn.addEventListener('click', () => {
      const text = globalCookiesText || liveCookiesText;
      if (!text) {
        showToast('No cookies available to export.', 'warning');
        return;
      }
      try {
    ...
    // Restore Global to use browser cookies (discard override)
    elements.globalRestoreLiveBtn.addEventListener('click', async () => {
      const ok = await showDialog({
        title: 'Discard Override?',
        message: 'This will delete your manually entered global cookies and restore dynamic live sync with the active browser tab.',
        confirmText: 'Discard & Restore',
      ...
      chrome.storage.local.set({ globalCookiesText: '' }, async () => {
        globalCookiesText = '';
        await refreshLiveCookiesState();
        showToast('Global profile restored to Live Sync.', 'success');
      });
    });

    // Check custom input modifications
    elements.globalCookies.addEventListener('input', () => {
      toggleGlobalMismatchHint(elements.globalCookies.value, liveCookiesText);
    });

    // Modal Live Sync trigger
    elements.profSyncCookiesBtn.addEventListener('click', async () => {
      const pattern = elements.profPattern.value.trim();
      if (!pattern) {
        showToast('Please specify a domain pattern first.', 'warning');
        return;
      }
      elements.profSyncCookiesBtn.disabled = true;
      elements.profSyncCookiesBtn.innerText = 'Syncing...';
      
      const cookies = await extractBrowserCookiesForDomain(pattern);
      elements.profCookies.value = cookies;
      
      elements.profSyncCookiesBtn.disabled = false;
      elements.profSyncCookiesBtn.innerText = '↻ Sync live cookies for domain';
      
      const count = countCookies(cookies);
      const dot = elements.profLiveCookieStatus.querySelector('.cookie-status-dot');
      const txt = elements.profLiveCookieStatus.querySelector('.cookie-status-text');
      
      if (count > 0) {
        elements.profLiveCookieStatus.className = 'cookie-status-indicator has-cookies';
        txt.innerText = `Found ${count} cookies in browser`;
        showToast(`Synced ${count} cookies for domain.`, 'success');
      } else {
        elements.profLiveCookieStatus.className = 'cookie-status-indicator no-cookies';
        txt.innerText = 'No cookies found in browser';
        showToast('No active cookies found in browser for this domain.', 'warning');
      }
    });

    // Modal domain pattern changes check
    elements.profPattern.addEventListener('input', debounce(() => {
      const pattern = elements.profPattern.value.trim();
      checkModalDomainCookies(pattern);
    }, 500));
...
function openProfileModal(id = null) {
  ...
  if (id) {
    ...
    if (prof) {
      ...
      if (prof.domainPattern) {
        checkModalDomainCookies(prof.domainPattern);
      }
    }
  } else {
    // New mode
...
function setupCookieFileReaders() {
  const handleFile = (fileInput, textareaElement, isGlobal = false) => {
    ...
    reader.onload = (e) => {
      ...
      if (isGlobal) {
        toggleGlobalMismatchHint(e.target.result, liveCookiesText);
      }
    };
  ...
  elements.globalCookieFile.addEventListener('change', () => {
    handleFile(elements.globalCookieFile, elements.globalCookies, true);
  });

  elements.profCookieFile.addEventListener('change', () => {
    handleFile(elements.profCookieFile, elements.profCookies, false);
  });
}
...
async function fetchVideoInfo(video, cardElement, matchedProfile) {
  ...
  try {
    const cookiesContent = await getCookiesForDownload(video, matchedProfile);
    const options = buildDownloadOptions(video, cardElement, matchedProfile);
    options.cookiesContent = cookiesContent;
